...
Servico* Registado::getServicoByID(unsigned int servID) const {
	for (unsigned int i = 0; i < historico.size(); i++) {
		if (historico[i]->getId() == servID) {
			return historico[i];
		}
	}
	return NULL;
...
bool Registado::adicionaServico(Servico* serv) {
	...
	//PART 2 MODIFICATION
	serv->setNome(this->getNome());
	serv->setNIF(this->getNIF());
	//-------------------
	historico.push_back(serv);
	return true;
}

Servico* Registado::getLastServico() const {
	...
	for (unsigned int i = 0; i < historico.size(); i++) {
		if (historico[i]->getData().ano > temp->getData().ano) {
			temp = historico[i];
		}
		else if (historico[i]->getData().ano == temp->getData().ano && historico[i]->getData().mes > temp->getData().mes) {
				temp = historico[i];
			}
		else if ( (historico[i]->getData().ano == temp->getData().ano) && 
					(historico[i]->getData().mes == temp->getData().mes) && 
						(historico[i]->getData().dia > temp->getData().dia)) {
				temp = historico[i];
			}
	}
	return temp;
